            ...
            if diff_x != 0:
                ...
                if diff_x < 0:
                    ...
                    for start in range(int(diff_x), 0, -1 * step):
                        ...
                        traces[trace_xi + 1].append(slope * start + y_p)
                        # if diff_y != 0:
                        #     slope = 1.0 * diff_x / diff_y
                        #     step = 1
                        #     if diff_y > 5:
                        #         step = int(diff_y / 5)
                        #     if diff_y < 0:
                        #         for start in range(int(diff_y), 0, step):
                        #             #matrix[y_p + start][int(round(slope * start + x_p))]= 1
                        #             traces[trace_xi].append(slope * start + x_p)
                        #             traces[trace_xi + 1].append(y_p + start)
                        #     else:
                        #         for start in range(int(diff_y), 0, -1*step):
                        #             #matrix[y_p + start][int(round(slope * start + x_p))]= 1
                        #             traces[trace_xi].append(slope * start + x_p)
                        #             traces[trace_xi + 1].append(y_p + start)
    return traces
...
def add_points_lin(traces):
    """
    This function finds 6 equidistant points between every pair of cosequtive points using
    linear interpolation.
...
def main():
    # this python file is to be called from another .py file. We don't run this standalone
    pass
